...
| Revoked or scoped authority | decisions and revalidation evaluate current DB assignments — roles and scopes, with validity windows and environment scopes (`test_tokens`, `test_scope_authority`, `test_policy_auth_refresh`) | Assignment administration is trusted-operator bound; changes are audited |
